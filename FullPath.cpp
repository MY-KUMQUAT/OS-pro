...
void Enter(struct PathNode *head,char *FILENAME)
{
    ...
    {
            if(strcmp(FILENAME,d_or_f[a].dir_list[i].filename)==0 && inode[d_or_f[a].dir_list[i].inode].inode_userID==userID)
            {
                //printf("find\n");
...
void dir(struct PathNode *head)
{
    ...
    for(int i=0;i<d_or_f[a].countcount;i++)
    {
        if(inode[dirlist[i].inode].inode_filetype==0 && inode[dirlist[i].inode].inode_userID==userID)
        {
    ...
    for(int i=0;i<d_or_f[a].countcount;i++)
    {
        if(inode[dirlist[i].inode].inode_filetype==1 && inode[dirlist[i].inode].inode_userID==userID)
        {
            cout<<dirlist[i].filename<<" ";
        }
    }
...
/*struct PathNode  *Copy_LinkedLisk(struct PathNode *COPY,struct PathNode *PASTE)
{
    struct PathNode *p1 = COPY,*p2 = PASTE;
    //p2 = InitPathNode();
    p1 = p1->next;p2 = p2->next;
    while(p1!=NULL)
    {
