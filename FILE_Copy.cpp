...
void copyfile(struct PathNode *head,char filename[])
{
    ...
    for(int i=0;i<d_or_f[a].countcount;i++)
    {
        ...
        {
            new_inode_filetype=inode[d_or_f[a].dir_list[i].inode].inode_filetype;  //文件类型
            new_inode_filelength=inode[d_or_f[a].dir_list[i].inode].inode_filelength;  //文件长度
            new_inode_limit=inode[d_or_f[a].dir_list[i].inode].inode_limit;  //文件读写权限
            new_inode_userID=inode[d_or_f[a].dir_list[i].inode].inode_userID;  //用户标识符
            ...
            for(int j=0;j<inode[d_or_f[a].dir_list[i].inode].inode_filelength;j++)  //读取文件内容
            {
                ...
                {
                    new_content[k] = storage[inode[d_or_f[a].dir_list[i].inode].inode_fileaddress[j]].txt_content[k];
                }
            }
            //cout<<endl;
            break;
